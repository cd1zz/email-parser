...
class EmailStructureExtractor:
    ...
    def _build_streamlined_email(self, message: Message, depth: int, verbose: bool = False) -> Dict[str, Any]:
        ...
        attachments = self._update_attachment_references(attachments, nested_emails)
    ...
    def _assign_nested_email_ids(self, nested_emails: List[Dict[str, Any]], depth: int) -> List[Dict[str, Any]]:
        """Assign unique IDs to nested emails and add source metadata."""
        for i, nested_email in enumerate(nested_emails):
            nested_email["id"] = f"nested_{depth}_{i}"
            
            # Add source attachment index if available
            if "source_attachment" in nested_email:
                nested_email["source_attachment_index"] = i
            
            # Recursively assign IDs to deeper nested emails
            if "nested_emails" in nested_email and nested_email["nested_emails"]:
                nested_email["nested_emails"] = self._assign_nested_email_ids(
                    nested_email["nested_emails"], depth + 1
                )
        
        return nested_emails

    def _update_attachment_references(self, attachments: List[Dict[str, Any]], nested_emails: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Update attachment objects to use references instead of full nested email objects."""
        # Create a mapping from source attachment to nested email ID
        attachment_to_id = {}
        for nested_email in nested_emails:
            source_attachment = nested_email.get("source_attachment")
            if source_attachment:
                attachment_to_id[source_attachment] = nested_email["id"]
        
        # Update attachments to use references
        for i, attachment in enumerate(attachments):
            if attachment.get("contains_email"):
                # Keep the nested email data for backward compatibility with test scripts
                # Don't remove the nested_email object - it's expected by test scripts
                
                # Add reference to the nested email ID
                attachment_name = attachment.get("name", "")
                nested_email_id = None
                
                # First try exact name match
                if attachment_name and attachment_name in attachment_to_id:
                    nested_email_id = attachment_to_id[attachment_name]
                
                # If no exact match, try other matching strategies
                if not nested_email_id:
                    for nested_email in nested_emails:
                        source_attachment = nested_email.get("source_attachment", "")
                        
                        # Strategy 1: Exact name match
                        if source_attachment == attachment_name:
                            nested_email_id = nested_email["id"]
                            break
                        
                        # Strategy 2: Match by attachment index (e.g., "attachment_1")
                        if source_attachment == f"attachment_{i+1}":
                            nested_email_id = nested_email["id"]
                            break
                        
                        # Strategy 3: Match by position if names don't match
                        if not attachment_name and source_attachment:
                            # Find the nested email that corresponds to this attachment by position
                            email_attachments = [att for att in attachments if att.get("contains_email")]
                            if len(email_attachments) == 1:  # Only one email attachment
                                nested_email_id = nested_email["id"]
                                break
                
                if nested_email_id:
                    attachment["nested_email_id"] = nested_email_id
                else:
                    # Log warning if we couldn't find a match
                    self.logger.warning(f"Could not find nested email ID for attachment: {attachment_name}")
                    
                    # Fallback: assign the first available nested email ID
                    if nested_emails:
                        attachment["nested_email_id"] = nested_emails[0]["id"]
    ...
    def _collect_urls_and_domains(self, all_emails: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Collect and deduplicate URLs and domains from all email layers."""
        from urllib.parse import urlparse
        
        all_urls = set()
        all_domains = set()
        
        def collect_from_email(email_entry):
            # Collect URLs from email body
            for url in email_entry.get("urls", []):
                if url and url.strip():
                    all_urls.add(url.strip())
                    try:
                        parsed = urlparse(url.strip())
                        if parsed.netloc:
                            all_domains.add(parsed.netloc.lower())
                    except:
                        pass
            
            # Collect URLs from attachments (document URLs)
            for attachment in email_entry.get("attachments", []):
                for doc_url in attachment.get("document_urls", []):
                    if doc_url and doc_url.strip():
                        all_urls.add(doc_url.strip())
                        try:
            ...
            # Collect URLs from document extracts
            for doc_extract in email_entry.get("document_extracts", []):
                for doc_url in doc_extract.get("urls", []):
                    if doc_url and doc_url.strip():
                        all_urls.add(doc_url.strip())
                        try:
                            parsed = urlparse(doc_url.strip())
            ...
            # Recursively collect from nested emails
            for nested_email in email_entry.get("nested_emails", []):
                collect_from_email(nested_email)
        
        # Collect from all emails
        for email_entry in all_emails:
            collect_from_email(email_entry)
        
        # Sort URLs and domains
        ...
        # Process shortened URLs and expand them
        shortened_urls_data = []
        for url in sorted_urls:
            if any(service in url.lower() for service in shortened_url_services):
                expanded_url = self._expand_shortened_url(url)
                shortened_urls_data.append({
                    "original": url,
        ...
        # Categorize domains
        trusted_domains = ['microsoft.com', 'outlook.com', 'gmail.com', 'yahoo.com', 'google.com']
        microsoft_domains = [domain for domain in sorted_domains if 'microsoft.com' in domain]
        email_provider_domains = [domain for domain in sorted_domains if any(provider in domain for provider in ['gmail.com', 'outlook.com', 'yahoo.com', 'google.com'])]
        external_domains = [domain for domain in sorted_domains if not any(trusted in domain for trusted in trusted_domains)]
        # Remove potentially_suspicious_domains as we're not making subjective judgments
        
        return {
            "urls": {
                "total_count": len(sorted_urls),
                "unique_urls": sorted_urls,
                "shortened_urls": shortened_urls_data,
                "by_domain": urls_by_domain
            },
            "domains": {
                "total_count": len(sorted_domains),
                "unique_domains": sorted_domains,
                "external_domains": external_domains,
                "by_category": {
                    "email_providers": email_provider_domains,
                    "microsoft_services": microsoft_domains
                }
            }
        }
    ...
    def _generate_summary(
        ...
        # Collect all URLs and domains from all layers
        url_domain_analysis = self._collect_urls_and_domains(all_emails)
        
        # Build enhanced summary with URL and domain deduplication
        summary = {
            "email_chain_length": len(all_emails),
            "total_emails": len(all_emails),
            "total_attachments": total_attachments,
            "total_depth": max([email_entry.get("level", 0) for email_entry in all_emails]) if all_emails else 0,
            
            # Enhanced URL section with deduplication
            "urls": url_domain_analysis["urls"],
            
            # Enhanced domain section with deduplication
            "domains": url_domain_analysis["domains"],
            
            # Keep existing summary fields for backward compatibility
            "attachment_types": sorted(list(attachment_types)),
            "domains_involved": sorted(list(domains)),  # Keep for backward compatibility
